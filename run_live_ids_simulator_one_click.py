...
def main() -> None:
    ...
    if missing:
        ...
        raise FileNotFoundError(
            "Train both models before running the live simulator. Missing files:\n"
            f"{missing_text}"
        )
    ...
    for model_path in [ml_model, fl_model]:
        ...
        if target_mode != "derived_multiclass":
            raise ValueError(
                ...
                "to regenerate multiclass models before starting the live simulator."
            )

    command = [
        sys.executable,
        str(workspace / "live_ids_simulator.py"),
        "--csv",
        ...
        "0.05",
        "--output-log",
        str(workspace / "simulation_logs" / "live_predictions.csv"),
    ]
